...
class SendLoginEmailTest(ZulipTestCase):
    ...
    def test_dont_send_login_emails_for_new_user_registration_logins(self) -> None:
        with self.settings(SEND_LOGIN_EMAILS=True):
            ...
            self.assertEqual(mail.outbox[0].subject, "Activate your Loop Zero account")
            self.assertEqual(len(mail.outbox), 1)
...
class TestBrowserAndOsUserAgentStrings(ZulipTestCase):
    ...
    def setUp(self) -> None:
        self.user_agents = [
            ...
            ('LoopZeroAndroid/1.0', 'Loop Zero', 'Android'),
            ('LoopZero/1.0.12 (Android 7.1.1)', 'Loop Zero', 'Android'),
            ('LoopZero/0.7.1.1 (iOS 10.3.1)', 'Loop Zero', 'iOS'),
